...
public:
	...
	virtual Tile*   HandleKeyboardInput(KeyboardMenuInputCode code);
	virtual bool    IsMenuEqual(Menu* that);
	virtual void    ScrollToHighlight(void);
	virtual Tile*   GetTileByIndex(int index, char isNotTileListIndex);
...
struct BSSimpleArray
{
	...
	virtual void Free(void* toFree);
	virtual void* ReAllocate(void* toResize, UInt32 size);
	...
	{
		UInt32 index = ThisCall<UInt32>(0x5E5350, this); // allocates new space for an appended element and returns the index of it
